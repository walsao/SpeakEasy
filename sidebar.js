...
document.addEventListener('DOMContentLoaded', function() {
  ...
  container.innerHTML = `
    <div class="tts-container ${settings.darkMode ? 'dark' : 'light'}">
      ...
      <div class="playback-controls">
        ...
        <button id="rewind-button" class="side-button rewind" title="Rewind 5 seconds">
        ...
        <button id="forward-button" class="side-button forward" title="Forward 5 seconds">
          <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none">
      ...
      <div id="reading-display" class="reading-display" style="display: none;">
        ...
        <div class="fullscreen-controls">
          <button id="fs-rewind-button" class="secondary-button" title="Rewind 5 seconds">
            <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none">
              <path d="M11 17l-5-5 5-5M18 17l-5-5 5-5"></path>
            </svg>
          </button>
          ...
          <button id="fs-forward-button" class="secondary-button" title="Forward 5 seconds">
            <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none">
              <path d="M13 17l5-5-5-5M6 17l5-5-5-5"></path>
            </svg>
          </button>
  ...
  // Add event listener for the main exit button
  document.getElementById('main-exit-button').addEventListener('click', exitExtension);
  
  // Add event listener for the fullscreen exit button (always visible in the corner)
  ...
  document.getElementById('rewind-button').addEventListener('click', function() {
    jumpPlayback(-5); // Jump back 5 seconds
  });
  
  document.getElementById('forward-button').addEventListener('click', function() {
    jumpPlayback(5); // Jump forward 5 seconds
  });
  ...
  document.getElementById('fs-play-button').addEventListener('click', function() {
    if (isPlaying) {
      stopSpeaking();
    } else {
      startSpeaking();
  ...
  document.getElementById('fs-pause-button').addEventListener('click', function() {
    if (isPaused) {
      resumeSpeaking();
    } else {
      pauseSpeaking();
  ...
  // IMPORTANT FIX: Make sure the fs-exit-button properly exits the extension
  ...
  function stopSpeaking() {
    ...
    isPlaying = false;
    isPaused = false;
    currentParagraphIndex = 0; // Reset to start
    ...
    const paragraphElements = document.querySelectorAll('#read-text p');
  ...
  function toggleFullScreen() {
    ...
    if (fullScreenMode) {
      ...
      // Show the standalone exit button in the corner
      ...
      // Hide the standalone exit button
  ...
  function exitReadingMode() {
    ...
    // Hide the fullscreen exit button
    const fullscreenExitBtn = document.getElementById('fullscreen-exit-btn');
    fullscreenExitBtn.style.display = 'none';
